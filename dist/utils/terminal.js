...
export function displayHeader() {
    console.clear();
    console.log(chalk.cyan(figlet.textSync('SUI Sniper', { font: 'Standard', horizontalLayout: 'full' })));
    console.log(boxen(chalk.bold('Entwickelt von DirtySanch3z'), {
        padding: 1,
        margin: 1,
        borderStyle: 'round',
        borderColor: 'cyan',
 ...
 */
export function displayActivePoolsTable(pools) {
    if (pools.length === 0) {
    ...
    for (const pool of pools) {
        ...
        let qualityColor;
        if (qualityScore >= 80)
            qualityColor = chalk.green;
        else if (qualityScore >= 60)
            qualityColor = chalk.cyan;
        else if (qualityScore >= 40)
            qualityColor = chalk.yellow;
        else if (qualityScore >= 20)
            qualityColor = chalk.hex('#FFA500'); // orange
        else
            qualityColor = chalk.red;
        table.addRow({
 ...
 */
export function displayPoolDetails(pool, status) {
    console.log(boxen(`${chalk.bold.cyan('Pool Details')}\n\n` +
 ...
 */
export function displaySnipingSummary(metrics) {
    const table = new Table({
 ...
 */
export function displayNewPoolAlert(pool, riskScore, isHoneypot) {
    console.log(boxen(`${chalk.bold.yellow('⚠️ NEUER POOL ENTDECKT ⚠️')}\n\n` +
        `${chalk.bold('Token:')} ${pool.tokenName} (${pool.tokenSymbol})\n` +
        `${chalk.bold('DEX:')} ${pool.dexType}\n` +
...
export function displayError(message) {
    console.log(boxen(chalk.bold.red(message), {
        padding: 1,
        margin: 1,
        borderStyle: 'round',
        borderColor: 'red',
...
export function displaySuccess(message) {
    console.log(boxen(chalk.bold.green(message), {
        padding: 1,
        margin: 1,
        borderStyle: 'round',
        borderColor: 'green',
 ...
 */
export function initializeUserInput(onCommand) {
    if (rl) {
        rl.close();
    }
...
export function displayHelp() {
    console.log(boxen(`${chalk.bold.cyan('Verfügbare Befehle')}\n\n` +
