...
async function retryWithBackoff(operation, attempts = RETRY_ATTEMPTS, initialDelay = RETRY_DELAY) {
    let lastError = null;
...
class Backtester {
    ...
    async initialize(startTime, endTime) {
    ...
    calculateProfit(transactionInfo) {
        try {
    ...
    calculateRiskAdjustment(transactionInfo) {
        // Basis-Risikofaktor
        let riskFactor = 1.0;
    ...
    async verifyIfWasScam(txDigest) {
    ...
    async calculateResults() {
        ...
        const stdDev = Math.sqrt(returns.reduce((a, b) => a + Math.pow(b - averageReturn, 2), 0) / returns.length);
...
export async function runBacktest(startTime, endTime) {
