...
async function analyzePool(pool) {
    try {
        ...
        const riskScore = Math.min(100, Math.max(0, scamRisk * 0.5 + // 50% weight on scam probability
...
async function scanNewPools() {
    try {
        ...
        await Promise.all(pools.map(pool => limit(async () => {
            if (knownPools.has(pool.poolAddress))
                return;
            // Analyze new pool
...
//# sourceMappingURL=poolScanner.js.map