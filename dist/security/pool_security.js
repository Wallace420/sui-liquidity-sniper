...
function asExtendedEffects(effects) {
    if (!effects)
        return null;
    return effects;
}
...
export async function checkPoolSecurity(poolId, dex) {
    try {
        ...
        const score = calculateSecurityScore(lpStatus, honeypotStatus, contractAnalysis, devWalletAnalysis, tokenAnalysis, poolMetrics);
        // Sammle Warnungen
        const warnings = collectWarnings(lpStatus, honeypotStatus, contractAnalysis, devWalletAnalysis, tokenAnalysis, poolMetrics);
        return {
            isSecure: score >= 80,
            score,
            warnings,
...
async function getTokenMetadata(coinType) {
    try {
...
async function checkLPStatus(poolId) {
    try {
        ...
        if (isLocked) {
            ...
            if (lockEvents.data.length > 0 && lockEvents.data[0].parsedJson) {
                const eventData = lockEvents.data[0].parsedJson;
...
async function checkHoneypot(poolId) {
    try {
...
async function analyzeContract(poolId) {
    try {
...
async function analyzeDevWallet(poolId) {
    try {
        // Hole Pool-Creator
        ...
        const poolCreations = transactions.data.filter(tx => {
            ...
            return events.some((e) => e.type.includes('::pool::CreatePoolEvent') ||
                e.type.includes('::factory::CreatePoolEvent'));
        }).length;
        // Berechne durchschnittliche Pool-Lebensdauer
        const poolLifetimes = await Promise.all(transactions.data
            .filter(tx => {
            const effects = tx.effects;
            const events = effects?.events || [];
            return events.some((e) => e.type.includes('pool'));
        })
            .map(async (tx) => {
            const effects = tx.effects;
            const events = effects?.events || [];
            const poolEvents = events.filter((e) => e.type.includes('pool'));
            if (poolEvents.length === 0)
                return 0;
            const eventData = poolEvents[0].parsedJson;
...
async function analyzeToken(poolId) {
    try {
        const pool = await SUI.client.getObject({
            id: poolId,
        ...
        const suspiciousTransfers = tokenTxs.data.filter(tx => {
            ...
            return moveEvents.some(e => {
                const transferData = e.parsedJson;
...
async function analyzePoolMetrics(poolId) {
    try {
        // Hole Pool-Details
        const pool = await SUI.client.getObject({
            id: poolId,
...
async function calculateTax(poolId, isBuy) {
    try {
        // Simuliere Trade
...
function calculateSecurityScore(lpStatus, honeypotStatus, contractAnalysis, devWalletAnalysis, tokenAnalysis, poolMetrics) {
    let score = 100;
    // LP Status (20%)
...
function collectWarnings(lpStatus, honeypotStatus, contractAnalysis, devWalletAnalysis, tokenAnalysis, poolMetrics) {
    const warnings = [];
    if (!lpStatus.isLocked) {
...
//# sourceMappingURL=pool_security.js.map