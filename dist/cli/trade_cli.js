...
async function showWalletInfo() {
    ...
    try {
        ...
        if (outgoingTxs.data.length > 0) {
            ...
            outgoingTxs.data.forEach((tx) => {
                console.log(`- ${tx.digest} (${new Date(Number(tx.timestampMs)).toLocaleString()})`);
            });
        }
        // Prüfe auf ausstehende Transaktionen
        const pendingTxs = [...incomingTxs.data, ...outgoingTxs.data].filter(tx => tx.effects?.status?.status && tx.effects.status.status !== 'success');
...
async function sendSUI(toAddress, amount) {
    try {
...
function shouldTrade(analysis) {
    return (analysis.scamScore < LIVE_TRADING_CONFIG.SAFETY_CHECKS.SCAM_SCORE_THRESHOLD &&
...
async function executeTrade(pool, analysis) {
...
async function removeLiquidity(poolId, amount) {
    try {
...
async function checkPoolSecurity(params) {
    try {
        // LP Token Burn Check
        ...
        const lpData = lpBurnCheck.data?.content;
...
async function checkBlacklist(tokenAddress) {
    // TODO: Implementiere Blacklist-Check gegen bekannte Scam-Token
    return false;
}
async function checkWhitelist(tokenAddress) {
...
async function analyzeBundleTransactions(poolId, tokenAddress) {
    try {
        ...
        const suspiciousCount = bundlePatterns.filter(p => p.hasMultipleActions && p.hasHighGas && p.isFromContract).length;
        return {
...
async function scamProbability(params) {
    ...
    const contractScore = ((security.ownershipRenounced ? 8 : 0) +
...
async function showPoolLinks(pool) {
    ...
    if (security.mintAuthority.isMintable) {
        ...
        if (security.mintAuthority.mintAuthorities.length > 0) {
            ...
            security.mintAuthority.mintAuthorities.forEach(auth => console.log(`   • ${auth}`));
        }
    }
    // Bundle Analyse
    console.log('\n📦 Bundle Analyse:');
...
rl.prompt();
//# sourceMappingURL=trade_cli.js.map