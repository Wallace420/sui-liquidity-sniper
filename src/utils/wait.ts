 ...
 */
function waitWithJitter(minMs: number, maxMs: number): Promise<number> {
  // Input validation
 ...
 * @returns A promise that resolves with the calculated delay in milliseconds
 */
function waitWithBackoff(baseMs: number, attempt: number, maxMs: number = 30000): Promise<number> {
  // Input validation
 ...
 */
function waitWithTimeout(ms: number, timeout: number): Promise<void> {
  // Input validation
