...
// Korrigiere den Import oder kommentiere ihn aus, wenn er nicht existiert
//import { getSuiPrice } from '@7kprotocol/sdk-ts';
import { getTrade } from '../db/trade.js';
...
// Definiere den TradeController-Typ
...
const tradeController: TradeController = importedTradeController as any;
...
bot.onText(/\/start/, async (msg: any) => {
  ...
  await bot.sendMessage(chatId, 
    '🤖 *Willkommen beim SUI Liquidity Sniper*\n\n' +
...
// Hauptmenü-Handler
// @ts-ignore: Property 'onText' existiert in der Typdefinition nicht
bot.onText(/🔄 Aktive Trades/, async (msg: any) => {
  const chatId = msg.chat.id;
...
// Wallet-Info Handler
// @ts-ignore: Property 'onText' existiert in der Typdefinition nicht
bot.onText(/👛 Wallet Info/, async (msg: any) => {
  const chatId = msg.chat.id;
...
// Performance-Handler
// @ts-ignore: Property 'onText' existiert in der Typdefinition nicht
bot.onText(/📊 Performance/, async (msg: any) => {
  const chatId = msg.chat.id;
...
// @ts-ignore: Property 'onText' existiert in der Typdefinition nicht
bot.onText(/⚙️ Einstellungen/, async (msg: any) => {
  const chatId = msg.chat.id;
  await bot.sendMessage(
    chatId,
...
// @ts-ignore: Property 'onText' existiert in der Typdefinition nicht
bot.onText(/🎯 Sniper Modus/, async (msg: any) => {
  const chatId = msg.chat.id;
  // @ts-ignore: Property 'getActiveTrades' existiert in der Typdefinition nicht
  const trades = tradeController.getActiveTrades();
...
// @ts-ignore: Property 'onText' existiert in der Typdefinition nicht
bot.onText(/⚠️ Risk Settings/, async (msg: any) => {
  const chatId = msg.chat.id;
...
// @ts-ignore: Property 'onText' existiert in der Typdefinition nicht
bot.onText(/🔒 Security/, async (msg: any) => {
  const chatId = msg.chat.id;
  ...
  switch (action) {
    case 'trade_details':
      ...
      if (tradeDetails) {
        ...
        await bot.sendMessage(
          chatId,
          `*Trade Details #${params[0].slice(0, 8)}*\n\n` +
          ...
          `Stabilität: ${analysis?.priceStability}%`,
          { parse_mode: 'Markdown' }
        );
      }
      break;
    ...
    case 'take_profit':
      ...
      await bot.sendMessage(
        chatId,
        '*Wählen Sie die Profit-Strategie:*',
        {
          parse_mode: 'Markdown',
          ...profitButtons
        }
    ...
    case 'profit_execute':
      ...
      const profitSuccess = await tradeController.takeProfits(txId, profitType as any);
      await bot.sendMessage(
        chatId,
        profitSuccess ? '✅ Profit erfolgreich realisiert' : '❌ Fehler beim Profit-Taking'
      );
      break;
    ...
    case 'set_trade_size':
      await bot.sendMessage(
        chatId,
        'Geben Sie die maximale Trade-Größe in SUI ein:\n' +
        'Format: `/set_max_trade 1.5`',
        { parse_mode: 'Markdown' }
      );
      break;
    ...
    case 'set_slippage':
      await bot.sendMessage(
        chatId,
        'Geben Sie den maximalen Slippage in % ein:\n' +
        'Format: `/set_slippage 1.0`',
        { parse_mode: 'Markdown' }
      );
      break;
    ...
    case 'set_scam_protection':
      await bot.sendMessage(
        chatId,
        'Geben Sie den Scam-Score Schwellenwert in % ein:\n' +
        'Format: `/set_scam_score 20`',
        { parse_mode: 'Markdown' }
      );
      break;
    ...
    case 'autopilot':
      ...
      if (autopilotTrade) {
        ...
        await bot.sendMessage(
          chatId,
          `🤖 Autopilot ${newStatus ? 'aktiviert' : 'deaktiviert'} für Trade #${tradeId.slice(0, 8)}`
        );
      }
      break;
    ...
    case 'close_trade':
      ...
      await bot.sendMessage(
        chatId,
        closingSuccess ? '✅ Trade erfolgreich geschlossen' : '❌ Fehler beim Schließen des Trades'
      );
      break;
    ...
    case 'sell_all':
      ...
      await bot.sendMessage(
        ...
        `❌ ${failCount} Trades fehlgeschlagen`,
        { parse_mode: 'Markdown' }
      );
      break;
    ...
    case 'pause_trading':
      ...
      await bot.sendMessage(
        chatId,
        '⏸️ Trading pausiert. Aktive Positionen bleiben bestehen.'
      );
      break;
    ...
    case 'set_lp_check':
      ...
      const lpLockSetting = LIVE_TRADING_CONFIG.SAFETY_CHECKS.hasOwnProperty('REQUIRED_LP_LOCK') 
        ? (LIVE_TRADING_CONFIG.SAFETY_CHECKS as any).REQUIRED_LP_LOCK 
        : false;
...
// @ts-ignore: Property 'onText' existiert in der Typdefinition nicht
bot.onText(/\/set_max_trade (.+)/, async (msg: any, match: any) => {
  const chatId = msg.chat.id;
  if (!match) return;
  ...
  await bot.sendMessage(
    chatId,
    `✅ Maximale Trade-Größe auf ${amount} SUI gesetzt`
  );
});

// LP Sicherheitseinstellungen
// @ts-ignore: Property 'REQUIRED_LP_LOCK' existiert nicht im Typ
const lpLockRequired = (LIVE_TRADING_CONFIG.SAFETY_CHECKS as any).REQUIRED_LP_LOCK ? 'Ja' : 'Nein';
...
export async function sendBuyMessage({
  ...
  await bot.sendMessage(process.env.TELEGRAM_GROUP_ID as string, message, {
    parse_mode: 'Markdown',
    ...buttons
  });
}
...
export async function sendUpdateMessage(params: any) {
  // Bestehende Implementierung...
}

export function sendErrorMessage(params: any) {
  // Bestehende Implementierung...
}

// Wallet Management Funktionen
// @ts-ignore: Property 'onText' existiert in der Typdefinition nicht
bot.onText(/\/send (.+) (.+)/, async (msg: any, match: any) => {
  const chatId = msg.chat.id;
  if (!match) return;
  ...
  try {
    ...
    await bot.sendMessage(
      chatId,
      '❌ Fehler bei der Transaktion: ' + (error instanceof Error ? error.message : 'Unbekannter Fehler')
    );
  }
});
...
// @ts-ignore: Property 'onText' existiert in der Typdefinition nicht
bot.onText(/⛔️ Stop Trading/, async (msg: any) => {
  const chatId = msg.chat.id;
