import { runBacktestWithConfig, BacktestConfig, AnalysisSummary } from './runBacktest';
...
async function runAnalysis(config: AnalysisConfig) {
  const now = Date.now();
...
function calculateProfitability(results: any[]) {
  return {
...
function calculateRiskMetrics(results: any[]) {
  return {
...
function calculateEfficiencyMetrics(results: any[]) {
  return {
...
function compareTimeframes(results: any[]) {
  ...
  return {
    ...
    bestTimeframe: timeframeComparisons.reduce((best, current) => 
      current.performance.riskAdjustedReturn > best.performance.riskAdjustedReturn ? current : best
    ),
...
function calculateProfitFactor(results: any[]): number {
...
function calculateROI(results: any[]): number {
...
function calculateProfitConsistency(results: any[]): number {
...
function calculateSharpeRatio(results: any[]): number {
...
function calculateVolatility(results: any[]): number {
  const returns = results.map(r => r.totalProfit);
  const mean = returns.reduce((sum, r) => sum + r, 0) / returns.length;
  return Math.sqrt(returns.reduce((sum, r) => sum + Math.pow(r - mean, 2), 0) / returns.length);
}

function calculateRiskRewardRatio(results: any[]): number {
...
function calculateDownsideDeviation(results: any[]): number {
  const returns = results.map(r => r.totalProfit);
...
function calculateGasEfficiency(results: any[]): number {
  const totalProfit = results.reduce((sum, r) => sum + r.totalProfit, 0);
  const totalGas = results.reduce((sum, r) => sum + r.gasUsed, 0);
  return totalGas === 0 ? 0 : totalProfit / totalGas;
}

function analyzeTimeframeTrends(comparisons: any[]) {
  // Sort by timeframe duration
  const sorted = [...comparisons].sort((a, b) => 
    parseInt(a.timeframe.split(' ')[1]) - parseInt(b.timeframe.split(' ')[1]));
...
function calculateConsistency(values: number[]): 'High' | 'Medium' | 'Low' {
