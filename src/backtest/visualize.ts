...
function generateChartHtml(chartData: ChartData[], titles: string[]): string {
  return `
<!DOCTYPE html>
<html>
<head>
...
function generateProfitabilityChart(results: TimedBacktestResult[]): ChartData {
...
function generateRiskMetricsChart(results: TimedBacktestResult[]): ChartData {
  const sortedResults = [...results].sort((a, b) => a.startTime - b.startTime);
...
function generateEfficiencyChart(results: TimedBacktestResult[]): ChartData {
  const sortedResults = [...results].sort((a, b) => a.startTime - b.startTime);
...
function generateScamDetectionChart(results: TimedBacktestResult[]): ChartData {
  const sortedResults = [...results].sort((a, b) => a.startTime - b.startTime);
...
async function visualizeResults(results: TimedBacktestResult[], config: VisualizationConfig) {
  try {
...
function generateColorScheme(numColors: number): string[] {
  const baseColors = [
    '#2ecc71', // Green
    '#3498db', // Blue
    '#e74c3c', // Red
...
async function main() {
  ...
  const mockResults: TimedBacktestResult[] = [
    /* ... mock data ... */
  ];
